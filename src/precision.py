...
regression amplifies TF32 rounding, so on a 3-view 480x640 scene:
  ...
  * TF32 vs full fp32, both in eager: 2.87 m max, 5.8e-02 m mean
  * an AOTInductor package vs eager, both TF32: 2.68 m max, 3.2e-02 m mean
  * an AOTInductor package vs eager, both full fp32: 6.8e-03 m max, 2.1e-04 m mean
...
compiled build numerically. (Measured on torch 2.6.0+cu126 / RTX 4070 Ti SUPER;
scripts/aot_compile_depth.py carries the full table.)
...
def set_fp32_precision(full_fp32: bool) -> None:
    """True: full fp32 everywhere. False: allow TF32 (faster, ~2.9 m different)."""
    if full_fp32:
